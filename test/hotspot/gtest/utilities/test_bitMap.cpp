...
class BitMapTestReverseClosure : public BitMapClosure {
private:
  idx_t* const _sequence;
  size_t const _length;
  idx_t        _index;
...
public:
  BitMapTestReverseClosure(size_t* sequence, size_t length) : _sequence(sequence), _length(length), _index(length) {}
...
public:
  ...
  static void test_with_closure(idx_t size, idx_t* seq, size_t seq_length) {
    ...
    // Iterate over the bits
    BitMapTestReverseClosure cl(seq, seq_length);
