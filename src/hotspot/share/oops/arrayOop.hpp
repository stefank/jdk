...
class arrayOopDesc : public oopDesc {
 ...
 public:
  ...
  static int base_offset_in_bytes(BasicType type) {
    return (int)to_bytes(header_size(type));
  }
  ...
  // array object type.
  static Words header_size(BasicType type) {
    Bytes typesize_in_bytes = Bytes(header_size_in_bytes());
    Words typesize_in_words = to_Words(typesize_in_bytes);
    return element_type_should_be_aligned(type)
      ? align_object_offset(typesize_in_words)
      : typesize_in_words;
  }
  ...
  static int32_t max_array_length(BasicType type) {
    ...
    const size_t max_element_words_per_size_t =
      align_down((SIZE_MAX/HeapWordSize - (size_t)header_size(type)), MinObjAlignment);
    ...
    if ((size_t)max_jint < max_elements_per_size_t) {
      ...
      return checked_cast<int32_t>(align_down(Words(max_jint) - header_size(type), MinObjAlignment));
    }
