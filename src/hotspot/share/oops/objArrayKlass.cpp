...
ArrayDescription ObjArrayKlass::array_layout_selection(const Klass* element, ArrayProperties props) {
  ...
  const ValueKlass* vk = ValueKlass::cast(element);
  ...
  if (props.is_null_restricted()) {
    if (props.is_non_atomic()) {
      ...
      if (vk->has_a(LayoutKind::NULL_FREE_NON_ATOMIC_FLAT)) {
        return ArrayDescription::flat(props, LayoutKind::NULL_FREE_NON_ATOMIC_FLAT);
      } else if (vk->has_a(LayoutKind::NULL_FREE_ATOMIC_FLAT)) {
...
ObjArrayKlass* ObjArrayKlass::allocate_klass_from_description(ArrayDescription ad, TRAPS) {
  ...
  if (ad.is_flat()) {
    ...
    return FlatArrayKlass::allocate_klass(element_klass(), ad._properties, ad.flat_layout_kind(), THREAD);
  } else {
