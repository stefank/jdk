...
void FieldGroup::add_flat_field(int idx, ValueKlass* vk, LayoutKind lk) {
  ...
  if (block->size() >= heapOopSize) {
    ...
    assert(!vk->contains_oops(), "Size of Value klass with oops should be >= heapOopSize");
    add_to_small_primitive_list(block);
  }
}
...
FieldLayout::FieldLayout(GrowableArray<FieldInfo>* field_info, Array<ValueFieldInfo>* value_field_info_array, ConstantPool* cp) :
  _field_info(field_info),
  _value_field_info_array(value_field_info_array),
  _cp(cp),
  _blocks(nullptr),
  _start(_blocks),
  _last(_blocks),
...
void FieldLayout::print(outputStream* output, bool is_static, const InstanceKlass* super, Array<ValueFieldInfo>* value_fields, bool dummy_field_is_reused_as_null_marker) {
  ResourceMark rm;
  LayoutRawBlock* b = _blocks;
  while(b != _last) {
    switch(b->block_kind()) {
...
void FieldLayoutBuilder::prologue() {
  _layout = new FieldLayout(_field_info, _value_field_info_array, _constant_pool);
  ...
  _static_layout = new FieldLayout(_field_info, _value_field_info_array, _constant_pool);
...
//   - primitive fields (both primitive types and flat value types) are allocated
...
// strategy is:
//   - big primitive fields (primitive types and flat value types larger
...
void FieldLayoutBuilder::compute_value_class_layout() {
  ...
  if (!_layout->super_has_nonstatic_fields()) {
    ...
    if (first_empty->offset() % payload_alignment != 0) {
      LayoutRawBlock* padding = new LayoutRawBlock(LayoutRawBlock::PADDING, payload_alignment - (first_empty->offset() % payload_alignment));
  ...
  if (first_field != nullptr) {
    layouts().set_payload_offset(_layout->first_field_block()->offset());
    layouts().set_payload_size_in_bytes(_layout->last_block()->offset() - _layout->first_field_block()->offset());
  } else {
    assert(_is_abstract_value, "Concrete value types must have at least one field");
    layouts().set_payload_offset(_layout->blocks()->size());
    layouts().set_payload_size_in_bytes(0);
  }
  ...
  if (!_is_abstract_value && vm_uses_flattening) { // Flat layouts are only for concrete value classes
    ...
    if (UseNullFreeNonAtomicValueFlattening && (!_must_be_atomic || _is_naturally_atomic)) {
      layouts().set_size_in_bytes_of(LayoutKind::NULL_FREE_NON_ATOMIC_FLAT, layouts().payload_size_in_bytes());
      layouts().set_alignment_of(LayoutKind::NULL_FREE_NON_ATOMIC_FLAT, layouts().payload_alignment());
    }

    // Next step is to compute the characteristics for a layout enabling atomic updates
    if (UseNullFreeAtomicValueFlattening) {
      const int payload_size = layouts().payload_size_in_bytes();
      int atomic_size = payload_size == 0 ? 0 : round_up_power_of_2(payload_size);
      if (atomic_size <= (int)MAX_ATOMIC_OP_SIZE) {
        layouts().set_size_in_bytes_of(LayoutKind::NULL_FREE_ATOMIC_FLAT, atomic_size);
        layouts().set_alignment_of(LayoutKind::NULL_FREE_ATOMIC_FLAT, atomic_size);
      }
    }
    ...
    if ((UseNullableAtomicValueFlattening || UseNullableNonAtomicValueFlattening)
      ...
      if (UseNullableNonAtomicValueFlattening) {
        layouts().set_size_in_bytes_of(LayoutKind::NULLABLE_NON_ATOMIC_FLAT, new_raw_size);
        layouts().set_alignment_of(LayoutKind::NULLABLE_NON_ATOMIC_FLAT, layouts().payload_alignment());
        layouts().set_null_marker_offset(null_marker_offset);
      }
      if (UseNullableAtomicValueFlattening) {
        ...
        if (nullable_atomic_size <= (int)MAX_ATOMIC_OP_SIZE) {
          layouts().set_size_in_bytes_of(LayoutKind::NULLABLE_ATOMIC_FLAT, nullable_atomic_size);
          layouts().set_alignment_of(LayoutKind::NULLABLE_ATOMIC_FLAT, nullable_atomic_size);
          layouts().set_null_marker_offset(null_marker_offset);
        }
      }
      if (this->null_marker_offset() == -1) { // No nullable layout has been accepted
    ...
    if (shift != 0) {
      if (required_alignment > layouts().payload_alignment() && !_layout->has_inherited_fields()) {
        ...
        layouts().set_payload_offset(_layout->first_field_block()->offset());
        assert(is_aligned(layouts().payload_offset(), required_alignment), "Fields should have been shifted to respect the required alignment");
        if (layouts().has_any(LayoutKind::NULLABLE_ATOMIC_FLAT, LayoutKind::NULLABLE_NON_ATOMIC_FLAT)) {
        ...
        layouts().set_alignment_of(LayoutKind::NULL_FREE_ATOMIC_FLAT, LayoutDescriptions::NoValue);
        if (layouts().has_a(LayoutKind::NULLABLE_ATOMIC_FLAT) &&
            !layouts().has_a(LayoutKind::NULLABLE_NON_ATOMIC_FLAT) &&
            !_is_empty_value_class) {  // empty values don't have a dedicated NULL_MARKER block
          _layout->remove_null_marker();
        ...
        layouts().set_alignment_of(LayoutKind::NULLABLE_ATOMIC_FLAT, LayoutDescriptions::NoValue);
      }
    } else {
      layouts().set_payload_alignment(required_alignment);
    }
    ...
    const int max_flat_layout_size_in_bytes =
        largest_layout_of(LayoutKind::NULLABLE_ATOMIC_FLAT,
                          LayoutKind::NULLABLE_NON_ATOMIC_FLAT,
                          LayoutKind::NULL_FREE_ATOMIC_FLAT);

    layouts().set_payload_size_in_bytes(
        MAX2(layouts().payload_size_in_bytes(),
             max_flat_layout_size_in_bytes));
  }
...
void FieldLayoutBuilder::epilogue() {
  ...
  if (PrintFieldLayout || (PrintValueLayout && (_has_flattenable_fields || _is_concrete_value || _is_abstract_value))) {
    ...
    const bool dummy_field_is_reused_as_null_marker = _is_empty_value_class && null_marker_offset() != -1;
    _layout->print(&st, false, _super_klass, _value_field_info_array, dummy_field_is_reused_as_null_marker);
    st.print_cr("Static fields:");
    _static_layout->print(&st, true, nullptr, _value_field_info_array, false);
    st.print_cr("Instance size = %d bytes", _info->_instance_size * wordSize);
    if (_is_concrete_value) {
      st.print_cr("First field offset = %d", layouts().payload_offset());
      st.print_cr("BUFFERED layout: %d/%d",
                  layouts().payload_size_in_bytes(), layouts().payload_alignment());
      layouts().print_on(st);
      if (null_marker_offset() != -1) {
        st.print_cr("Null marker offset = %d", null_marker_offset());
      }
