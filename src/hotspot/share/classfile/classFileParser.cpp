...
void ClassFileParser::apply_parsed_class_metadata(
  ...
  this_klass->set_value_field_info_array(_value_field_info_array);
...
void ClassFileParser::set_fast_acmp_members(ValueKlass* vk) const {
  ...
  for (int i = 0; i < _layout_info->_nonoop_acmp_map->length(); i++) {
    int piece_start = _layout_info->_nonoop_acmp_map->at(i)._offset - _layout_info->_value_layouts.payload_offset();
  ...
  if (mask == 0) {
    ...
    int offset = _layout_info->_value_layouts.payload_offset() - trailing_zeroes / BitsPerByte;
  ...
  if (mask == 0) {
    ...
    int offset = _layout_info->_value_layouts.payload_offset() - leading_zeroes / BitsPerByte;
    assert(offset >= 0, "fast acmp path shouldn't load before the object");
    vk->set_fast_acmp_offset(offset);
    vk->set_fast_acmp_mask(mask);
  }
...
void ClassFileParser::fill_instance_klass(InstanceKlass* ik,
    ...
    for (int i = 0; i < _temp_field_info->length(); i++) {
      ...
      if (fieldinfo.access_flags().is_static() && fieldinfo.field_flags().is_null_free_value_type()) {
        ...
        if (name == _class_name) {
          ...
          _value_field_info_array->adr_at(fieldinfo.index())->set_klass(ValueKlass::cast(ik));
        }
        assert(_value_field_info_array->adr_at(fieldinfo.index())->klass()->is_value_klass(), "Must be");
      }
    }
  }
  ...
  assert(nullptr == _value_field_info_array, "invariant");
  ...
  if (is_concrete_value_class()) {
    ...
    vk->set_layouts(_layout_info->_value_layouts);
...
ClassFileParser::~ClassFileParser() {
  ...
  if (_value_field_info_array != nullptr) {
    MetadataFactory::free_array<ValueFieldInfo>(_loader_data, _value_field_info_array);
  }
...
void ClassFileParser::post_process_parsed_stream(const ClassFileStream* const stream,
  ...
  FieldLayoutBuilder lb(class_name(), loader_data(), super_klass(), _cp, /*_fields*/ _temp_field_info,
      ...
      _must_be_atomic, _layout_info, _value_field_info_array);
  lb.build_layout();

  // If it turned out that we didn't flatten any of the fields, we deallocate
  // the array of ValueFieldInfo since it isn't needed, and so it isn't
  // transferred to the allocated InstanceKlass.
  if (_value_field_info_array != nullptr && !(_layout_info->_has_flat_fields || _has_null_restricted_static_fields)) {
    MetadataFactory::free_array<ValueFieldInfo>(_loader_data, _value_field_info_array);
    _value_field_info_array = nullptr;
  }
    ...
    if (Signature::has_envelope(sig)) {
      ...
      if (name == _class_name) {
        if (fieldinfo.field_flags().is_null_free_value_type() && !is_concrete_value_class()) {
          ...
          // Dummy setting to trigger the allocation of the value_field_info array -
          // the real pointer will be set later in ::fill_instance_klass, once the ValueKlass has been allocated.
          set_value_field_info_klass(fieldinfo.index(), nullptr, CHECK);
        }
        continue;
      }
      if (PreloadClasses && is_class_in_loadable_descriptors_attribute(sig)) {
        ...
        if (klass != nullptr) {
          if (klass->is_value_klass()) {
            set_value_field_info_klass(fieldinfo.index(), ValueKlass::cast(klass), CHECK);
            log_info(class, preload)("Preloading of class %s during loading of class %s "
...
void ClassFileParser::set_value_field_info_klass(int field_index, ValueKlass* vk, TRAPS) {
  assert(field_index >= 0 && field_index < java_fields_count(), "IOOB: 0 <= %d < %d", field_index, (int)java_fields_count());

  // The array of ValueFieldInfo is allocated on demand. This way the array is
  // never allocated for an InstanceKlass which has no need for this information.
  if (_value_field_info_array == nullptr) {
    _value_field_info_array = MetadataFactory::new_array<ValueFieldInfo>(_loader_data,
                                                                         java_fields_count(),
                                                                         CHECK);
  }

  // Set the Klass for the field's index
  _value_field_info_array->adr_at(field_index)->set_klass(vk);
}
