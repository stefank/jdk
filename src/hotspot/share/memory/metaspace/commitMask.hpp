...
class CommitMask : public CHeapBitMap {
  ...
  const Words  _words_per_bit;

  // Given an offset, in words, into the area, return the number of the bit
  // covering it.
  static idx_t bitno_for_word_offset(Words offset, Words words_per_bit) {
  ...
  idx_t bitno_for_address(const MetaWord* p) const {
    ...
    const Words off = pointer_delta(p,_base);
    return bitno_for_word_offset(off, _words_per_bit);
  }

  static idx_t mask_size(Words word_size, Words words_per_bit) {
  ...
  void check_range(const MetaWord* start, Words word_size) const;
...
public:
  ...
  CommitMask(const MetaWord* start, Words word_size);
  ...
  Words get_committed_size_in_range(const MetaWord* start, Words word_size) const {
    DEBUG_ONLY(check_range(start, word_size));
    assert(word_size > Words(0), "zero range");
  ...
  Words get_committed_size() const {
  ...
  Words mark_range_as_committed(const MetaWord* start, Words word_size) {
    DEBUG_ONLY(check_range(start, word_size));
    assert(word_size > Words(0), "zero range");
    const idx_t b1 = bitno_for_address(start);
    const idx_t b2 = bitno_for_address(start + word_size);
    if (b1 == b2) { // Simple case, 1 granule
      ...
      return was_committed ? _words_per_bit : Words(0);
    }
  ...
  Words mark_range_as_uncommitted(const MetaWord* start, Words word_size) {
    DEBUG_ONLY(check_range(start, word_size));
    assert(word_size > Words(0), "zero range");
    const idx_t b1 = bitno_for_address(start);
    const idx_t b2 = bitno_for_address(start + word_size);
    if (b1 == b2) { // Simple case, 1 granule
      ...
      return was_committed ? Words(0) : _words_per_bit;
    }
