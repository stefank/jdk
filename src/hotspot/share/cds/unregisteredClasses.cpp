...
Handle UnregisteredClasses::get_url_classloader(Symbol* path, TRAPS) {
  ...
  if (url_classloader_ptr != NULL) {
    ...
    OopHandle url_classloader_oh(Universe::vm_global(), url_classloader());
    _url_classloader_table->put(path, url_classloader_oh);
    path->increment_refcount();
    return Handle(THREAD, url_classloader_oh.resolve());
  }
}
