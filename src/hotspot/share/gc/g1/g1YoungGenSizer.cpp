...
G1YoungGenSizer::G1YoungGenSizer() : _sizer_kind(SizerDefaults),
  ...
  if (FLAG_IS_CMDLINE(NewSize)) {
    _min_desired_young_length = MAX2((uint) (NewSize / untype(HeapRegion::GrainBytes)),
                                     1U);
    if (FLAG_IS_CMDLINE(MaxNewSize)) {
      _max_desired_young_length =
                             MAX2((uint) (MaxNewSize / untype(HeapRegion::GrainBytes)),
                                  1U);
...
void G1YoungGenSizer::adjust_max_new_size(uint number_of_heap_regions) {
  ...
  Bytes max_young_size = result * HeapRegion::GrainBytes;
  if (untype(max_young_size) != MaxNewSize) {
