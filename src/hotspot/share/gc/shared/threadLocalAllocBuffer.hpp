...
private:
  ...
  Words     _desired_size;                       // desired size   (including alignment_reserve)
  Words     _refill_waste_limit;                 // hold onto tlab if free() is larger than this
  Bytes     _allocated_before_last_gc;           // total bytes allocated up until the last gc
  Bytes     _bytes_since_last_sample_point;      // bytes since last sample point.

  static Words    _max_size;                          // maximum size of any TLAB
  static Words    _reserve_for_allocation_prefetch;   // Reserve at the end of the TLAB
  ...
  void set_desired_size(Words desired_size)      { _desired_size = desired_size; }
  void set_refill_waste_limit(Words waste)       { _refill_waste_limit = waste;  }

  Words initial_refill_waste_limit();

  static int    target_refills()                 { return _target_refills; }
  Words initial_desired_size();
...
public:
  ...
  static Words max_size()                       { assert(_max_size != Words(0), "max_size not set up"); return _max_size; }
  static Bytes max_size_in_bytes()              { return to_Bytes(max_size()); }
  static void set_max_size(Words max_size)      { _max_size = max_size; }
  ...
  Words desired_size() const                     { return _desired_size; }
  Words used() const                             { return pointer_delta(top(), start()); }
  Bytes used_bytes() const                       { return in_Bytes(pointer_delta(top(), start(), 1)); }
  Words free() const                             { return pointer_delta(end(), top()); }
  // Don't discard tlab if remaining space is larger than this.
  Words refill_waste_limit() const               { return _refill_waste_limit; }
  Bytes bytes_since_last_sample_point() const    { return _bytes_since_last_sample_point; }
  ...
  static Words end_reserve();
  static Words alignment_reserve()              { return align_object_size(end_reserve()); }
  static Bytes alignment_reserve_in_bytes()     { return to_Bytes(alignment_reserve()); }

  // Return tlab size or remaining space in eden such that the
  // space is large enough to hold obj_size and necessary fill space.
  // Otherwise return 0;
  inline Words compute_size(Words obj_size);

  // Compute the minimal needed tlab size for the given object size.
  static inline Words compute_min_size(Words obj_size);

  // Record slow allocation
  inline void record_slow_allocation(Words obj_size);
  ...
  void fill(HeapWord* start, HeapWord* top, Words new_size);
  void initialize();

  void set_back_allocation_end();
  void set_sample_end(bool reset_byte_accumulation);

  static Words refill_waste_limit_increment();
