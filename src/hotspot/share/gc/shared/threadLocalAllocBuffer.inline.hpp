...
inline HeapWord* ThreadLocalAllocBuffer::allocate(Words size) {
  invariants();
  HeapWord* obj = top();
...
inline Words ThreadLocalAllocBuffer::compute_size(Words obj_size) {
  // Compute the size for the new TLAB.
  // The "last" tlab may be smaller to reduce fragmentation.
  // unsafe_max_tlab_alloc is just a hint.
  const Words available_size = to_Words(Bytes(Universe::heap()->unsafe_max_tlab_alloc(thread())));
  Words new_tlab_size = MIN3(available_size, desired_size() + align_object_size(obj_size), max_size());
...
inline Words ThreadLocalAllocBuffer::compute_min_size(Words obj_size) {
  const Words aligned_obj_size = align_object_size(obj_size);
  const Words size_with_reserve = aligned_obj_size + alignment_reserve();
  return MAX2(size_with_reserve, heap_word_size(MinTLABSize));
}

void ThreadLocalAllocBuffer::record_slow_allocation(Words obj_size) {
