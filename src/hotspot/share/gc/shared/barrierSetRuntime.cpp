...
JRT_LEAF(void, BarrierSetRuntime::value_copy(address src, address dst, ValueFieldLayoutInfo* li))
  FlatValuePayload src_payload = FlatValuePayload::construct_from_parts(src, li->klass(), li->kind());
  FlatValuePayload dst_payload = FlatValuePayload::construct_from_parts(dst, li->klass(), li->kind());
