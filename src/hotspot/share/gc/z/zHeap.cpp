...
void ZHeap::object_iterate(ObjectClosure* object_cl, bool visit_weaks) {
  ...
  ZHeapIterator iter(1 /* nworkers */, visit_weaks, ZHeapIterator::ObjectVisitLocation::AtFollow);
...
void ZHeap::object_and_field_iterate_for_verify(ObjectClosure* object_cl, OopFieldClosure* field_cl, bool visit_weaks) {
  ...
  // requirements on the passed in closures. That is because the closures will
  // be called during root iteraiton, which takes various locks.
  ZHeapIterator iter(1 /* nworkers */, visit_weaks, ZHeapIterator::ObjectVisitLocation::AtMark);
...
ParallelObjectIteratorImpl* ZHeap::parallel_object_iterator(uint nworkers, bool visit_weaks) {
  assert(SafepointSynchronize::is_at_safepoint(), "Should be at safepoint");
  return new ZHeapIterator(nworkers, visit_weaks, ZHeapIterator::ObjectVisitLocation::AtFollow);
}
