...
void VirtualMemory::update_peak(Bytes size) {
  Bytes peak_sz = peak_size();
  while (peak_sz < size) {
    Bytes old_sz = Atomic::cmpxchg(&_peak_size, peak_sz, size, memory_order_relaxed);
    if (old_sz == peak_sz) {
      break;
    } else {
      peak_sz = old_sz;
...
static bool is_mergeable_with(CommittedMemoryRegion* rgn, address addr, Bytes size, const NativeCallStack& stack) {
  return rgn->adjacent_to(addr, size) && rgn->call_stack()->equals(stack);
}

static bool is_same_as(CommittedMemoryRegion* rgn, address addr, Bytes size, const NativeCallStack& stack) {
...
static bool try_merge_with(LinkedListNode<CommittedMemoryRegion>* node, address addr, Bytes size, const NativeCallStack& stack) {
  if (node != nullptr) {
    CommittedMemoryRegion* rgn = node->data();
...
bool ReservedMemoryRegion::add_committed_region(address addr, Bytes size, const NativeCallStack& stack) {
...
bool ReservedMemoryRegion::remove_uncommitted_region(LinkedListNode<CommittedMemoryRegion>* node,
  ...
  if (rgn->base() == addr ||
    ...
    Bytes exclude_size = pointer_delta_bytes(rgn->end(), addr);
    rgn->exclude_region(addr, exclude_size);

    // higher part
    address high_base = addr + size;
    Bytes   high_size = pointer_delta_bytes(top, high_base);
...
bool ReservedMemoryRegion::remove_uncommitted_region(address addr, Bytes sz) {
  assert(addr != nullptr, "Invalid address");
  ...
  while (head != nullptr) {
    ...
    if (crgn->contain_address(addr)) {
      ...
      if (crgn->contain_address(end - 1)) {
        ...
        Bytes size = pointer_delta_bytes(crgn->end(), del_rgn.base());
      ...
      Bytes size = pointer_delta_bytes(del_rgn.end(), crgn->base());
...
Bytes ReservedMemoryRegion::committed_size() const {
...
bool VirtualMemoryTracker::add_reserved_region(address base_addr, Bytes size,
...
void VirtualMemoryTracker::set_reserved_region_type(address addr, MEMFLAGS flag) {
  ...
  ReservedMemoryRegion   rgn(addr, Bytes(1));
  ReservedMemoryRegion*  reserved_rgn = _reserved_regions->find(rgn);
  if (reserved_rgn != nullptr) {
...
bool VirtualMemoryTracker::add_committed_region(address addr, Bytes size,
...
bool VirtualMemoryTracker::remove_uncommitted_region(address addr, Bytes size) {
  assert(addr != nullptr, "Invalid address");
  assert(size > Bytes(0), "Invalid size");
  assert(_reserved_regions != nullptr, "Sanity check");
...
bool VirtualMemoryTracker::remove_released_region(address addr, Bytes size) {
  assert(addr != nullptr, "Invalid address");
  assert(size > Bytes(0), "Invalid size");
  assert(_reserved_regions != nullptr, "Sanity check");
  ...
  if (reserved_rgn->base() == addr ||
    ...
    address high_base = addr + size;
    ReservedMemoryRegion high_rgn(high_base, in_Bytes(top - high_base),
      *reserved_rgn->call_stack(), reserved_rgn->flag());

    // use original region for lower region
    reserved_rgn->exclude_region(addr, in_Bytes(top - addr));
...
bool VirtualMemoryTracker::split_reserved_region(address addr, Bytes size, Bytes split) {

  ReservedMemoryRegion  rgn(addr, size);
  ReservedMemoryRegion* reserved_rgn = _reserved_regions->find(rgn);
  assert(reserved_rgn->same_region(addr, size), "Must be identical region");
  assert(reserved_rgn != nullptr, "No reserved region");
  assert(reserved_rgn->committed_size() == Bytes(0), "Splitting committed region?");
...
public:
  ...
  bool next_committed(address& start, Bytes& size);
private:
  address end() const { return _start + _size; }
};

bool RegionIterator::next_committed(address& committed_start, Bytes& committed_size) {
  if (end() <= _current_start) return false;

  const size_t page_sz = os::vm_page_size();
  const Bytes current_size = in_Bytes(end() - _current_start);
  if (os::committed_in_range(_current_start, untype(current_size), committed_start, (size_t&)committed_size)) {
    assert(committed_start != nullptr, "Must be");
    assert(committed_size > Bytes(0) && is_aligned(committed_size, os::vm_page_size()), "Must be");
  ...
  bool do_allocation_site(const ReservedMemoryRegion* rgn) {
    if (rgn->flag() == mtThreadStack) {
      ...
      while (itr.next_committed(committed_start, committed_size)) {
        ...
        assert(committed_size > Bytes(0), "Should not be 0");
        ...
        if (stack_bottom + stack_size < committed_start + committed_size) {
          committed_size = pointer_delta_bytes(stack_bottom + stack_size, committed_start);
        }
